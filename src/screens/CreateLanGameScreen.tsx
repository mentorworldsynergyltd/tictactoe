...
export function CreateLanGameScreen() {
  const navigate = useNavigate()
  const { hostGame, status, errorMessage, resetError } = useLanGame()
  ...
  return (
    <div className="mx-auto flex min-h-full w-full max-w-md flex-col justify-center px-6 py-10">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-secondary">Create LAN Game</p>
        <h1 className="mt-1 text-2xl font-black text-ink">Host a local match</h1>

        <Card className="mt-6">
          <label htmlFor="host-name" className="block text-xs font-bold uppercase tracking-wide text-muted">
            Your Name
          </label>
          <input
            id="host-name"
